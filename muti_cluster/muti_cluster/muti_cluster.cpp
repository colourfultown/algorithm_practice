...
{
	string strin = "G:/taxi/c/144_quantian_OD/51_OD1.txt";//输入的变换后的坐标初始值
	string strin_2 = "G:/taxi/c/144_quantian_OD/51_OD.txt";//输入的经纬度的坐标
	string strout = "G:/taxi/c/O/51OD_80_20_16_gezi.txt";//输出的格子与分类标号
	string strout_2 = "G:/taxi/c/O/51OD_80_20_16.txt";//输出的经纬度与分类标号
	
	init_vec(strin);
	cout << "初始化结束" << endl;
	/*cout << vec.size() << endl;
	cout << vec[vec.size()-1].x << endl;
	cout  << vec[vec.size() - 2].x << endl;
	cout  << vec[vec.size() - 3].x << endl;*/
	point_ p1;
	...
	pro(strout,5,100);//如果格子内点数少于这个阈值，删除格子
	                  //第二个参数是限定最多多少个格子聚在一起
	cout << "格子分类结束" << endl;
	cal_rou();
	cout << "密度计算结束" << endl;
	...
	cout <<"分类数：" <<c << endl;
	/*cout << ans[471].clafi << c << endl;
	cout << ans[472].x << c << endl;
	cout << ans[474].y << c << endl;
	cout << ans[475].banjing << c << endl;*/

	
	//string strout = "G:/taxi/c/out2.txt";
	//write(res, strout);
	getchar();
	return 0;
}
